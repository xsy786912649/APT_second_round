...
for observation_number in [2]:
    ...
    plt.title("Graph 1, Very weak SIEM system", fontsize=16)
    plt.subplots_adjust(left=0.098, right=0.993, top=0.94, bottom=0.110)
    plt.savefig('./figures/esitmation_error_graph1'+"_observation_weaker"+str(observation_number)+"_"+str(zomm_image)+'.pdf') 
    plt.show()
